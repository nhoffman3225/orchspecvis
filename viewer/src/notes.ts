 ...
 * value = part index + 1 (0 = no note) in +-widthBins around each note's fundamental.
 */
export function rasterizeNotes(
  ...
  for (const i of which) {
    ...
    if (f1 <= f0) continue;
    const c = (notes.midi[i]! - m.fmin_midi) * k;
...
export function rasterizeF0(
  ...
  visible: (stem: number) => boolean = () => true,
): Uint8Array {
  const nb = m.n_bins;
  const k = m.bins_per_octave / 12;
  const step = 2 ** level;
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let f = 0; f < pageFrames; f++) {
      ...
      for (let g = g0; g < Math.min(frames0, g0 + step); g++) {
        ...
        if (hz <= 0) continue;
        const c = (69 + 12 * Math.log2(hz / 440) - m.fmin_midi) * k;
        const b0 = Math.max(0, Math.round(c - widthBins));
        const b1 = Math.min(nb - 1, Math.round(c + widthBins));
...
/** Keep page cells where mask != 0; everything else (overtones, noise) goes to 0. */
export function applyMask(page: Uint8Array, mask: Uint8Array): Uint8Array {
  const out = new Uint8Array(page.length);
  for (let i = 0; i < page.length; i++) if (mask[i]) out[i] = page[i]!;
  return out;
}
